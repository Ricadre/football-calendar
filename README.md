...
AC米兰男足一线队与兰州陇原竞技的非官方球迷日历。比赛日程包含对阵双方、开球时间、赛事、场馆、提醒及比分。
...
- 米兰场馆以官方赛程为准。兰州来源缺少逐场场馆，`data/venues.json` 记录经过查找的球队常用主场及证据。已知场馆直接显示球场名，日历备注说明「按主队常用主场列示」，保留参考来源与临时换场说明；不因缺少本场专门公告而在名称后统一加「本场待确认」。查不到场馆或明确需要重新安排时才显示待确认。有本场战报或公告的，按比赛 ID 记录具体场馆；赛事页面提供的 SportsEvent 场馆也会优先采用。具体比赛场馆与球队常用主场在备注中区分，不将主场参考表述成已获本场官方确认。
- 初始核实时间：2026-09-08；后续赛程 52 场，其中米兰 44 场、兰州 8 场。未公布的后续杯赛、下赛季赛程在来源发布后才能加入。
- `data/fixtures.json` 记录每个来源的最后成功读取时间、失败原因和稳定事件版本。

## 自动更新

GitHub Actions 计划每 30 分钟检查一次（每小时第 7、37 分钟），也可以手动运行。每次都读取最新赛程和比赛状态，覆盖赛前、赛中、终场后及次日；待定时间同样持续检查，最近 14 天的已结束比赛继续核对比分更正。日历声明的建议刷新周期同步改为 30 分钟。

GitHub 调度可能延迟或跳过，iPhone 的实际订阅刷新由系统决定；ICS 的刷新周期仅是建议，不能强制手机每 30 分钟拉取，也不保证即时比分。

兰州同时读取未开赛、进行中和已结束的比赛，并按已保存的比赛 ID 持续查询单场页面。球队列表暂时缺项时仍会检查单场详情；已确认时间不会因此降级成“时间待定”。单场读取失败时保留该场已确认内容，其余比赛继续更新，页面和工作流会报告错误。赛程大批消失时拒绝覆盖，避免误删。

两队均提取来源的比赛状态和主客队比分：进行中标记“进行中”，终场后标题如“【已结束】【客场】广州蒲公英 1–2 兰州陇原竞技”。未开赛的 0–0 占位值不会当作比分。赛果更新沿用原 UID，递增 SEQUENCE 与 LAST-MODIFIED，并移除赛前提醒；同样内容重复抓取不会产生新版本。已确认的终场结果不会被旧的未开赛缓存覆盖。最近 14 天的比赛保留，较早比赛逐步移除。
...
- `config.py` 集中管理默认北京时间 09:00、赛前 30 分钟与前晚 21:00 的提醒，以及时区、显示名称、保留天数和数据源。关闭早提醒可设 `MORNING_REMINDER = None`。修改 `REFRESH_MINUTES` 时需同步修改 workflow 的 cron。
